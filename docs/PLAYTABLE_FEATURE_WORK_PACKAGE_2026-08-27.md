...
## Post-ship fix round (S11.1) — Adrian-reported, 2026-08-28: DONE, deployed

Adrian used the live table after S11 shipped and reported two things directly: most of the newer controls "don't have a place in the ui", and placing a light "triggers no response". Both were real, reproduced with a real browser against a disposable stack (not assumed from code reading), not caching or a one-off fluke — see the diagnostic screenshots sent in-conversation.

**Root cause 1 — widget content buried below an unmarked scroll fold.** Every slice from S05 on kept appending rows to the Tokens/Turn Order floating panels without anyone revisiting the original `max-height: 40%` budget from the 2026-08-23 table refactor. Worse than just "too short": `.floating`'s `overflow: auto` lived on the WHOLE panel including its own `<h3>` header, so scrolling to reach new content scrolled the title and collapse-toggle out of view too. Fix: restructured `.floating` to flex layout with the header pinned (`flex: 0 0 auto`) and only `.widget-body` scrolling (`flex: 1 1 auto; overflow-y: auto`), added a CSS-only scroll-shadow gradient cue (the standard "fades at a genuinely overflowing edge, disappears when there's nothing more to scroll" technique, colors matched per-container — `--panel` for `.floating` panels, the sidebar's own `rgba(5,11,22,0.96)` for `#chatLog`/`#activityLog`/`#diceLog`/`.tab-panel`, `.table-sheet`'s own `rgba(7,14,28,0.98)` on mobile), bumped the height budget moderately (40%→50% base, proportionally at the three responsive breakpoints), and added a labeled "Sicht & Nebel" sub-heading so a partial scroll reveals a named section instead of anonymous buttons after "Kampf beenden". Applied to every container found doing the same thing, not just the two that prompted the report: Tokens, Turn Order, the three sidebar logs, the Tools tab, and the mobile `.table-sheet`.

**Root cause 2 — light markers visually near-invisible against the dark theme.** The click DID register (real DB row, zero JS/console errors) — the marker rendered at 10–18% fill opacity against `#0c1426`, reading as a dark smudge rather than a light. Fix: roughly doubled fill/stroke opacity, enlarged the center dot 5px→8px with a `drop-shadow` glow, added a one-shot placement flash animation, and a confirmation toast ("Lichtquelle platziert."/"Wand platziert.").

**A real regression caught mid-fix, before it shipped:** `.floating`'s new `display: flex` silently won a CSS specificity tie against the browser's own `[hidden] { display: none }` rule (both 0,0,1,0 — the class rule's source position broke the tie), which un-hid `#tokenCreatePanel` whenever it should have stayed hidden. Fixed with an explicit `.floating[hidden] { display: none; }` (0,0,2,0, wins unconditionally). Caught by screenshot, not by any existing test — every prior robot flow only ever asserted the `hidden` DOM attribute/property, never that an element was actually invisible on screen, which is exactly the same category of gap that let the original bug ship.

**Verification methodology change driven by this round:** an adversarial multi-agent review (independent code-regression pass, fresh-eyes visual judgment of the light fix, and a systematic sweep of the rest of the app for the same buried-content pattern) surfaced two things the first fix pass missed entirely:

1. **A genuine regression the restructure itself introduced:** `#tokenCreatePanel` is the one `.floating` panel whose content was never wrapped in `.widget-body` — moving `.floating` from self-scrolling to only-`.widget-body`-scrolling silently removed its scroll mechanism entirely, worse than the bug being fixed (on mobile, where this panel is height-capped, its Platzieren/Abbrechen buttons could clip with literally no way to reach them). Fixed by wrapping its content div in `class="widget-body"` too — needs no toggle/collapse behavior to apply correctly, that only activates via the parent's `.collapsed` class, which this panel never gets.
2. **`#visibleTokensPopover` ("Sichtbare Tokens") was completely unreachable** — confirmed independently on both desktop and mobile, unrelated to the widget-scroll fix. Unlike every sibling popover, `_showVisibleTokensForSelected` never called any positioning code at all; being `position: fixed` but nested deep in a scrolled widget tree, it rendered 170–700px below the actual viewport. A DM-facing tool that had been silently broken since it shipped in S10, and would never have been caught by the S10 robot flow's own assertion (`wait_for_selector(":not([hidden])")`, which checks CSS visibility, not viewport position — the exact same blind spot behind the original complaint).

Fixing #2 properly (a shared `_positionPopoverNear(popover, x, y, {anchorAbove})` helper, wired into all five dynamically-positioned popovers: conditions, loot, wall-edit, light-edit, visible-tokens) surfaced a **third, deeper root cause** while debugging why the new clamping math still put the popover off-screen: this app's book-page-flip shell (`.book-dashboard-camera` / `.book-dashboard-page` in `vtt/static/css/book-scene.css`) sets `will-change: transform` for animation performance, which per spec silently re-anchors the CSS containing block for every `position: fixed` descendant away from the true viewport. Every popover's `window.innerWidth`/`innerHeight`-based positioning math had been computing against the wrong reference frame — not a new bug from this fix, but the same adversarial sweep independently found `#conditionsPopover` clipping 34px off the desktop right edge and rendering above the mobile viewport's top, both consistent with this same root cause. Fixed generically: `_positionPopoverNear` now walks up from the popover to find the actual containing-block-establishing ancestor (any of `transform`/`will-change:transform`/`perspective`/`filter` not `none`) and clamps against *that* element's bounding rect instead of assuming the viewport, falling back to the viewport when no such ancestor exists.

Also fixed, lower-priority, found during the same pass: a cosmetic double-opacity-compositing seam where the new scroll-shadow's flat `background-color: var(--panel)` stacked on top of `.floating`'s own already-translucent background — removed the redundant layer (the gradient's own color stops already provide one correct, non-doubled layer).

Permanent regression coverage added to the robot fleet (`widget_discoverability`, 16th flow): header-stays-pinned-after-scroll, every previously-buried control's real bounding-box reachability (not DOM presence), the Sicht & Nebel sub-heading text, the light marker's real computed fill alpha and dot radius, the `#tokenCreatePanel` hidden-attribute regression guard, its restored `.widget-body` scroll mechanism, and `#visibleTokensPopover`'s on-screen position plus a real click on its own close button (which times out if the popover is actually off-screen, unlike a hidden-attribute assertion).

Verification: 549 pytest passed, 0 failed (unchanged — this round is template/CSS/JS + a robot flow, no new pytest tests); full 16-flow robot fleet, 0 findings, confirmed stable across repeated runs.
