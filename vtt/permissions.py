...
def _resolve_authenticated_user():
    """Resolve the current JWT-backed user for protected platform/admin routes."""
...
def _platform_role_level(platform_role):
    """Normalize configured platform role levels to an integer."""
    raw_level = PLATFORM_ROLES.get(platform_role, 0)
    if isinstance(raw_level, dict):
        return int(raw_level.get('level', 0) or 0)
    try:
...
def has_platform_role_level(min_level):
    ...
    def decorator(f):
        ...
        def decorated_function(*args, **kwargs):
            ...
            current_level = _platform_role_level(user.platform_role)
...
def can_create_campaign(user):
    ...
    """
    if not _is_authenticated_user(user) or user.is_suspended:
        return False
