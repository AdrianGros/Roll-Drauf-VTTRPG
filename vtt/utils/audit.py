...
def _extract_performer_id(performer):
    """Resolve a stable actor ID without relying on Flask-Login-style helpers."""
...
def log_audit(action, resource_type=None, resource_id=None, details=None, performed_by=None):
    ...
    Returns:
        AuditLog instance
    """
    from vtt.models import AuditLog

    performer = performed_by or current_user
    performer_id = _extract_performer_id(performer)
    if performer_id is None:
        raise RuntimeError('Audit performer could not be resolved')
    ...
    log = AuditLog(
        ...
        timestamp=utcnow(),
        performed_by_id=performer_id if performed_by is not None else None
    )
