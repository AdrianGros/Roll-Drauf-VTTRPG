...
def _serialize_campaign_character(character: Character):
    payload = character.serialize(include_details=True)
    payload.update(
        {
            "user_id": character.user_id,
            "owner_username": character.user.username if character.user else None,
        }
    )
    return payload
...
def _get_campaign_character_or_404(campaign_id: int, character_id: int):
    ...
    if not character:
        return None, (jsonify({"error": "character not found in this campaign"}), 404)
    return character, None


def _can_manage_session_assignment(campaign: Campaign, user_id: int, character: Character) -> bool:
    if _is_dm(campaign, user_id):
        return True
    return character.user_id == user_id


def _session_assignment_editable(game_session: GameSession) -> bool:
    runtime_status = str(game_session.get_runtime_status() or "").strip().lower()
    return runtime_status not in {"in_progress", "active", "live", "ended", "completed"}
...
def get_campaign(campaign_id):
    ...
    characters = (
        Character.query.filter_by(campaign_id=campaign.id, deleted_at=None)
        .order_by(Character.name.asc(), Character.created_at.asc())
        .all()
    )
    assignments = (
        SessionCharacterAssignment.query.join(
            GameSession,
            SessionCharacterAssignment.game_session_id == GameSession.id,
        )
        .filter(GameSession.campaign_id == campaign.id)
        .order_by(
            SessionCharacterAssignment.game_session_id.asc(),
            SessionCharacterAssignment.created_at.asc(),
        )
        .all()
    )
    ...
    return jsonify(
        {
            ...
            "characters": [_serialize_campaign_character(character) for character in characters],
            "session_character_assignments": [assignment.serialize() for assignment in assignments],
        }
    ), 200
...
@campaigns_bp.route("/campaigns/<int:campaign_id>/sessions/<int:session_id>/characters", methods=["POST"])
@limiter.limit("80 per hour")
@jwt_required()
def assign_character_to_session(campaign_id, session_id):
    """Assign a campaign-linked character to a session during pre-table prep."""
    user, error = _get_current_user()
    if error:
        return error
    ...
    if error:
        return error
    if not _session_assignment_editable(game_session):
        return jsonify({"error": "session assignment is only available before table entry or while paused"}), 409
    ...
    character, error = _get_campaign_character_or_404(campaign.id, character_id)
    if error:
        return error
    if not _can_manage_session_assignment(campaign, user.id, character):
        return jsonify({"error": "forbidden"}), 403

    existing = SessionCharacterAssignment.query.filter_by(
        game_session_id=game_session.id,
        character_id=character.id,
    ).first()
    if existing:
        return jsonify({"error": "character already assigned to this session"}), 409

    assignment = SessionCharacterAssignment(
        game_session_id=game_session.id,
        character_id=character.id,
    ...
    return jsonify(
        {
            ...
            "character": _serialize_campaign_character(character),
        }
    ), 201


@campaigns_bp.route("/campaigns/<int:campaign_id>/sessions/<int:session_id>/characters/<int:character_id>", methods=["DELETE"])
@limiter.limit("120 per hour")
@jwt_required()
def unassign_character_from_session(campaign_id, session_id, character_id):
    """Remove a character assignment from a session during pre-table prep."""
    user, error = _get_current_user()
    if error:
        return error
    ...
    assignment = SessionCharacterAssignment.query.filter_by(
        game_session_id=game_session.id,
        character_id=character_id,
    ).first()
    if not assignment:
    ...
    if character and not _can_manage_session_assignment(campaign, user.id, character):
        return jsonify({"error": "forbidden"}), 403
    if not character and not _is_dm(campaign, user.id):
        return jsonify({"error": "forbidden"}), 403

    db.session.delete(assignment)
    db.session.commit()
    return jsonify({"message": "character unassigned", "character_id": character_id}), 200
