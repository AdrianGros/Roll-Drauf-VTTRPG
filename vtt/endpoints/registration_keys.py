...
from vtt.models import RegistrationKey, User
...
def _internal_bot_secret() -> str:
    return str(current_app.config.get("DISCORD_BOT_SHARED_SECRET") or "").strip()


def _require_internal_bot_auth():
    configured_secret = _internal_bot_secret()
    presented_secret = request.headers.get("X-RollDrauf-Bot-Secret", "").strip()

    if not configured_secret or not presented_secret:
        return jsonify({"error": "internal bot auth missing"}), 401

    if not hmac.compare_digest(configured_secret, presented_secret):
        return jsonify({"error": "internal bot auth invalid"}), 403

    return None


def _build_assignment_batch_id(discord_user_id: str) -> str:
    return f"discord-assign-{discord_user_id}"


def _serialize_key_assignment(key: RegistrationKey) -> dict:
    return {
        "id": key.id,
...
@registration_keys_bp.route('/internal/registration-keys/issue', methods=['POST'])
@limiter.limit("30 per hour")
def issue_internal_assignment_key():
    """Issue or return a single-use registration key assigned to a Discord user."""
    auth_failure = _require_internal_bot_auth()
    if auth_failure is not None:
        return auth_failure

    data = request.get_json() or {}
    discord_user_id = str(data.get('discord_user_id') or '').strip()
    tier = str(data.get('tier') or 'player').strip().lower()
    expires_in_days = data.get('expires_in_days')
    assigned_by = str(data.get('assigned_by') or '').strip()

    if not discord_user_id.isdigit():
        return jsonify({'error': 'discord_user_id must be a numeric Discord snowflake'}), 400

    valid_tiers = ['free', 'player', 'dm', 'headmaster']
    if tier not in valid_tiers:
        return jsonify({'error': f'tier must be one of {valid_tiers}'}), 400

    batch_id = _build_assignment_batch_id(discord_user_id)
    existing_key = (
        RegistrationKey.query
        .filter_by(key_batch_id=batch_id, is_revoked=False)
        .filter(RegistrationKey.uses_remaining > 0)
        .order_by(desc(RegistrationKey.created_at))
        .first()
    )
    if existing_key is not None:
        return jsonify({
            'created': False,
            'assignment': _serialize_key_assignment(existing_key),
        }), 200

    expires_at = None
    if expires_in_days is not None:
        if not isinstance(expires_in_days, int) or expires_in_days < 1 or expires_in_days > 365:
            return jsonify({'error': 'expires_in_days must be between 1 and 365'}), 400
        from datetime import timedelta
        expires_at = utcnow() + timedelta(days=expires_in_days)
    ...
    current_app.logger.info(
        "internal key issued assignment_type=discord discord_user_id=%s key_id=%s tier=%s assigned_by=%s",
        discord_user_id,
        key.id,
        tier,
        assigned_by or "-",
    ...
    return jsonify({
        'created': True,
        'assignment': _serialize_key_assignment(key),
    }), 201


@registration_keys_bp.route('/internal/registration-keys/assigned/<discord_user_id>', methods=['GET'])
@limiter.limit("120 per hour")
def list_internal_assignment_keys(discord_user_id):
    """Return unconsumed registration keys assigned to a Discord user."""
    auth_failure = _require_internal_bot_auth()
    if auth_failure is not None:
        return auth_failure
    ...
    if not normalized_user_id.isdigit():
        return jsonify({'error': 'discord_user_id must be numeric'}), 400

    batch_id = _build_assignment_batch_id(normalized_user_id)
    keys = (
        RegistrationKey.query
        .filter_by(key_batch_id=batch_id, is_revoked=False)
        .filter(RegistrationKey.uses_remaining > 0)
    ...
    return jsonify({
        ...
        'assignments': [_serialize_key_assignment(key) for key in keys],
    }), 200
...
def export_batch_pdf(batch_id):
    ...
    """
    from vtt.pdf_templates.key_batch import generate_key_batch_pdf
    from vtt.models import AppThemeSettings
